        ...
        with torch.no_grad():
            ...
            for dro_kspace, csmap, ground_truth, dro_grasp_img, mask, grasp_path, raw_kspace, raw_grasp_img, raw_csmaps in tqdm(val_dro_loader, desc="Step 0 Validation Evaluation"):
                ...
                if global_rank == 0 or not config['training']['multigpu']:
                    ...
                    ssim, psnr, mse, lpips, dc_mse, dc_mae, recon_corr, grasp_corr = eval_sample(dro_kspace, csmap, ground_truth, x_recon, eval_physics, mask, dro_grasp_img, acceleration, int(N_spokes), eval_dir, label='val0', device=device, cluster=cluster, dro_eval=True)
                    ...
                    dc_mse_raw, dc_mae_raw = eval_sample(raw_kspace, raw_csmaps, ground_truth, raw_x_recon, eval_physics, mask, raw_grasp_img, acceleration, int(N_spokes), eval_dir, label='val0', device=device, cluster=cluster, dro_eval=False, grasp_path=grasp_path, raw_slice_idx=raw_grasp_slice_idx)
                ...
                with torch.no_grad():
                    for val_dro_kspace_batch, val_csmap, val_ground_truth, val_dro_grasp_img, val_mask, grasp_path, val_raw_kspace, val_raw_grasp_img, val_raw_csmaps in tqdm(val_dro_loader):
                        ...
                        try:
                            ...
                            ## Evaluation
                            if global_rank == 0 or not config['training']['multigpu']:
                                ssim, psnr, mse, lpips, dc_mse, dc_mae, recon_corr, _ = eval_sample(val_dro_kspace_batch, val_csmap, val_ground_truth, val_x_recon, eval_physics, val_mask, val_dro_grasp_img, acceleration, int(N_spokes), eval_dir, f'epoch{epoch}', device, cluster=cluster, dro_eval=True)
                                ...
                                # raw k-space eval
                                dc_mse_raw, dc_mae_raw = eval_sample(val_raw_kspace, val_raw_csmaps, val_ground_truth, val_raw_x_recon, eval_physics, val_mask, val_raw_grasp_img, acceleration, int(N_spokes), eval_dir, label=f'epoch{epoch}', device=device, cluster=cluster, dro_eval=False, grasp_path=grasp_path, raw_slice_idx=raw_grasp_slice_idx)
        ...
        with torch.no_grad():
            ...
            for eval_config in STRESS_TEST_PLAN:
                ...
                for csmap, ground_truth, dro_grasp_img, mask, grasp_path, raw_kspace, raw_csmaps, raw_grasp_img in tqdm(eval_spf_loader, desc="Variable Spokes Per Frame Evaluation"):
                    ...
                    ## Evaluation
                    ssim, psnr, mse, lpips, dc_mse, dc_mae, recon_corr, grasp_corr = eval_sample(kspace, csmap, ground_truth, x_recon, physics, mask, dro_grasp_img, acceleration, int(spokes), eval_dir, f"{spokes}spf", device, cluster=cluster, dro_eval=True)
                    ...
                    dc_mse_raw, dc_mae_raw = eval_sample(raw_kspace, raw_csmaps, ground_truth, raw_x_recon, physics, mask, raw_grasp_img, acceleration, int(N_spokes), eval_dir, label=f"{spokes}spf", device=device, cluster=cluster, dro_eval=False, grasp_path=grasp_path, raw_slice_idx=raw_grasp_slice_idx)
