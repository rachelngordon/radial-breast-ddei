...
def main():
    ...
    accum_steps = int(config["training"].get("gradient_accum_steps", 1))
    if accum_steps < 1:
        accum_steps = 1
    if (dist.is_initialized() if config["training"]["multigpu"] else False) and accum_steps > 1:
        if dist.get_rank() == 0:
            print(f"Using gradient accumulation: {accum_steps} micro-batches per optimizer step.")
    ...
    if (epochs + 1) == start_epoch:
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            for batch_idx, (measured_kspace, csmap, N_samples, N_spokes, N_time) in enumerate(train_loader_tqdm):  # measured_kspace shape: (B, C, I, S, T)
                ...
                # Use DDP no_sync() on micro-batches that WON'T step the optimizer
                do_sync = ((batch_idx + 1) % accum_steps == 0)
                sync_ctx = (
                    model.no_sync()
                    if (config['training']['multigpu'] and not do_sync)
                ...
                try:
                    ...
                    # Only clip/step/zero on accumulation boundary (or last batch)
                    is_last_batch = (batch_idx == len(train_loader) - 1)
                    if do_sync or is_last_batch:
