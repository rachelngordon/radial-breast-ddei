...
# ... (fft, ifft, normalize_sens_maps_sos1 functions are unchanged) ...
...
def normalize_sens_maps_sos1(sens_maps: torch.Tensor, coil_dim: int = -1, eps: float = 1e-8) -> torch.Tensor:
    with torch.no_grad():
...
class DigitalPhantomGenerator:
    ...
    Supports both Cartesian and Radial k-space trajectories.
    """
    def __init__(
        self,
        phantom_dims: List[int] = [64, 64, 32, 16, 8],  # [nx, ny, nz, nt, nc]
        
        ### MODIFIED: Add trajectory type switch ###
        trajectory_type: str = 'cartesian', # 'cartesian' or 'radial'
        
        ### MODIFIED: Group trajectory-specific params ###
        # Cartesian-specific
        cartesian_acceleration: List[float] = [1.0, 1.0, 1.0],  # [ax, ay, az]
        # Radial-specific
        radial_num_spokes_per_frame: int = 32,
        ...
        ### MODIFIED: Store trajectory info ###
        self.trajectory_type = trajectory_type.lower()
        self.cartesian_acceleration = cartesian_acceleration
        self.radial_num_spokes = radial_num_spokes_per_frame

        if self.trajectory_type not in ['cartesian', 'radial']:
            raise ValueError(f"trajectory_type must be 'cartesian' or 'radial', but got {trajectory_type}")
        ...
        logger.info(f"Trajectory type set to: {self.trajectory_type}")

    # The `generate` method using the fixed combination logic is now the correct one.
    # Note: I am reverting to the last working version of this method.
    def generate(self) -> Dict[str, torch.Tensor]:
        ...
        phantom_mask = (torch.abs(static_background_img) > 1e-6).float()
        dynamic_C_cpu, dynamic_B_cpu = self._solve_pdes_finite_difference(nx, ny, nz, nt, self.phys_params)
        dynamic_C, dynamic_B = dynamic_C_cpu.to(self.device), dynamic_B_cpu.to(self.device)
        masked_dynamic_B = dynamic_B * phantom_mask.unsqueeze(-1)
        magnitude_combined = torch.abs(static_background_img).unsqueeze(-1) + dynamic_C + masked_dynamic_B
        static_phase = torch.angle(static_background_img).unsqueeze(-1)
        dynamic_phase = self.phys_params["K_phase"] * dynamic_C
        phase_combined = static_phase + dynamic_phase
        gt_coilless_image_combined = magnitude_combined * torch.exp(1j * phase_combined)
        gt_coilless_image_final = gt_coilless_image_combined.permute(3, 0, 1, 2)
        ...
        coil_images_dense_CTXYZ = coil_images_dense.permute(4, 3, 0, 1, 2)
        kspace_cartesian_dense_CTKXYZ = fft(coil_images_dense_CTXYZ, dim=(-3, -2, -1))
        return {
            "gt_coilless_image": gt_coilless_image_final, "sensitivity_maps": sens_maps_norm,
            "coil_images": coil_images_dense_CTXYZ, "kspace_cartesian_dense": kspace_cartesian_dense_CTKXYZ,
        }

    ### MODIFIED: This is the main method for generating data for a model ###
    def generate_accelerated_kspace(self) -> Dict[str, torch.Tensor]:
        """
        Generates phantom data and simulates an accelerated k-space acquisition
        using the specified trajectory type.
        """
        full_data = self.generate()
        coil_images = full_data['coil_images']
        nx, ny, nz, nt, nc = self.dims

        # --- TRAJECTORY GENERATION SWITCH ---
        if self.trajectory_type == 'cartesian':
            k_trajectories, k_integer_indices = self._generate_phantom_cartesian_trajectory(
                img_dims_xyz=[nx, ny, nz], num_time_points=nt, acceleration_factors=self.cartesian_acceleration
            )
        elif self.trajectory_type == 'radial':
            k_trajectories, k_integer_indices = self._generate_phantom_radial_trajectory(
                img_dims_xyz=[nx, ny, nz], num_time_points=nt, num_spokes_per_frame=self.radial_num_spokes
            )
        else:
            raise NotImplementedError(f"Trajectory type '{self.trajectory_type}' not implemented.")

        # The rest of the logic is trajectory-agnostic thanks to NUFFT
        logger.info(f"Sampling phantom k-space at {self.trajectory_type} trajectory points using NUFFT.")
        ...
        return {
            ...
            "k_integer_indices": k_integer_indices, # Will be None for radial
    ...
    def _generate_phantom_cartesian_trajectory(self, img_dims_xyz: List[int], num_time_points: int, acceleration_factors: List[float]) -> Tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    ### NEW: Radial Trajectory Generation Function ###
    def _generate_phantom_radial_trajectory(
        self,
        ...
        """
        Generates a synthetic 3D "stack-of-stars" radial k-space trajectory.
        """
        nx_img, ny_img, nz_img = img_dims_xyz
        num_points_per_spoke = nx_img
        logger.info(
            f"Generating phantom RADIAL trajectory: ImgDims={img_dims_xyz}, "
            f"Spokes/Frame={num_spokes_per_frame}, T={num_time_points}"
        )
        golden_angle = np.pi * (3.0 - np.sqrt(5.0))
        radius = torch.linspace(-1, 1, num_points_per_spoke, dtype=torch.float32, device=self.device) * np.pi
        kz_indices = torch.arange(-nz_img / 2, nz_img / 2, dtype=torch.float32, device=self.device)
        kz_scaled = kz_indices * (2 * np.pi / nz_img)
        
        # num_k_points_frame = num_spokes_per_frame * nz_img
        num_k_points_frame = num_spokes_per_frame * nz_img * num_points_per_spoke
        k_trajectories_all_t = torch.zeros((num_time_points, num_k_points_frame, 3), dtype=torch.float32, device=self.device)

        total_spoke_count = 0
        for t in range(num_time_points):
            frame_coords = []
            for i_spoke in range(num_spokes_per_frame):
                angle = total_spoke_count * golden_angle
                kx_spoke = radius * torch.cos(torch.tensor(angle))
                ky_spoke = radius * torch.sin(torch.tensor(angle))
                for kz_val in kz_scaled:
                    spoke_coords = torch.stack([
                        torch.full_like(kx_spoke, kz_val),
                        ky_spoke,
                        kx_spoke,
                    ], dim=1)
                    frame_coords.append(spoke_coords)
                total_spoke_count += 1
            k_trajectories_all_t[t] = torch.cat(frame_coords, dim=0)

        logger.info(f"Generated phantom radial trajectory (scaled, ZYX order): {k_trajectories_all_t.shape}")
        return k_trajectories_all_t, None # No integer indices for radial

    # This is the last working version of the PDE solver.
    def _solve_pdes_finite_difference(self, nx, ny, nz, nt_out, params, static_mask_cpu=None):
        ...
        if static_mask_cpu is not None:
            phantom_mask = static_mask_cpu.to(sim_device)
        else: # Fallback if no mask is provided, though it shouldn't happen with the new logic
            phantom_mask = torch.ones(nz, ny, nx, device=sim_device)
        ...
        results_C_rearranged = rearrange(results_C, "t z y x -> x y z t").cpu()
        results_B_rearranged = rearrange(results_B, "t z y x -> x y z t").cpu()
        return results_C_rearranged, results_B_rearranged
