...
class RadialDCESample(NamedTuple):
...
class RadialDCEDataTransform:
    def __init__(
        self,
        use_seed: bool = True,
        time_window: int = 8,
        padding_size: Tuple = (640, 640),
        normalize: bool = True):
    ...
    def __call__(
        ...
        # mask = to_tensor(mask if len(mask.shape) > 2 else np.expand_dims(mask, axis=-1))
        ...
        # calculate acc
        rate = total_spokes / spokes_per_frame
        # slice = mask[:, :, 0]#mask.squeeze()
        ...
        #! 6. Apply time window
        # NOTE: do not need to apply the time window if using 8 timeframes
