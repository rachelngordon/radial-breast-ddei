...
class SimulatedSPFDataset(Dataset):
    ...
    def __init__(self, root_dir, model_type, patient_ids):
        ...
        # self.ktraj, self.dcomp, self.nufft_ob, self.adjnufft_ob = prep_nufft(N_SAMPLES_SPOKE, self.spokes_per_frame, self.num_frames)
            
        # # Instantiate the physics model for this specific case
        # self.physics = MCNUFFT(self.nufft_ob.to(device), self.adjnufft_ob.to(device), self.ktraj.to(device), self.dcomp.to(device))
