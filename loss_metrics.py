...
#     This class performs the following steps:
#     1. Converts the (B, 2, H, W, T) real/imag tensor to a (B, 1, T, H, W) magnitude tensor.
#     2. Normalizes the magnitude video to the range [-1, 1] required by LPIPS. It uses
#        the target's min/max for consistent scaling.
#     3. Reshapes the video into a batch of 2D images: (B*T, 1, H, W).
#     4. Repeats the single channel to create a 3-channel image for LPIPS.
#     5. Computes the LPIPS score and returns the mean.
#     """
#     def __init__(self, net_type='alex'):
#         super().__init__()
#         # Initialize LPIPS. We set normalize=False because we'll do it manually.
#         self.lpips = LearnedPerceptualImagePatchSimilarity(net_type=net_type, normalize=False)
...
#         # --- 2. Normalize to [-1, 1] based on the target's range ---
#         # Rearrange to easily find min/max over all spatial and temporal dimensions
#         target_mag_flat = rearrange(target_mag, 'b c h w t -> b (c h w t)')
#         min_vals = target_mag_flat.min(dim=1, keepdim=True)[0]
#         max_vals = target_mag_flat.max(dim=1, keepdim=True)[0]

#         # Reshape min/max for broadcasting
#         min_vals = rearrange(min_vals, 'b (c h w t) -> b c h w t', c=1, h=1, w=1, t=1)
#         max_vals = rearrange(max_vals, 'b (c h w t) -> b c h w t', c=1, h=1, w=1, t=1)
...
#         # Apply normalization
#         pred_norm = 2 * (pred_mag - min_vals) / range_vals - 1
#         target_norm = 2 * (target_mag - min_vals) / range_vals - 1

#         # --- 3. Reshape for LPIPS: Fold Batch and Time dimensions ---
#         # Input shape: (B, 1, H, W, T) -> rearrange -> (B, 1, T, H, W) -> flatten -> ((B*T), 1, H, W)
#         pred_flat = rearrange(pred_norm, 'b c h w t -> (b t) c h w')
#         target_flat = rearrange(target_norm, 'b c h w t -> (b t) c h w')

#         # --- 4. Repeat single channel to get 3 channels ---
#         pred_flat_rgb = pred_flat.repeat(1, 3, 1, 1)
#         target_flat_rgb = target_flat.repeat(1, 3, 1, 1)
