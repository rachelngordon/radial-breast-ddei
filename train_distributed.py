...
def main():
    ...
    model = ArtifactRemovalLSFPNet(lsfp_backbone, block_dir if global_rank==0 else None).to(device)
    ...
    if args.from_checkpoint and global_rank == 0:
        checkpoint_file = f'output/{exp_name}/{exp_name}_model.pth'
        model.module, optimizer, start_epoch, train_curves, val_curves, eval_curves = load_checkpoint(model.module, optimizer, checkpoint_file)
    else:
        start_epoch = 1
    ...
    if (epochs + 1) == start_epoch:
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            if epoch % save_interval == 0 and global_rank == 0:
